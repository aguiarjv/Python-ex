...
def parse_command_line():
    ...
    parser.add_argument('-d', '--date', metavar = ('month', 'day', 'year'), nargs = 3, type = int, help = "formatted date M D Y (i.e. 03 28 1998)")
...
def create_date(datelist, surprise):
    ...
    """
    early_date = date(day = 16, month = 6, year = 1995)
    today_date = date.today()

    if (datelist):
        try:
            n_date = date(month = datelist[0], day = datelist[1], year = datelist[2])
            if (n_date < early_date or n_date > today_date):
                raise ValueError("Not a valid date!")
            else:
                sel_date = date(day = n_date.day, month = n_date.month, year = n_date.year)
                return sel_date
        ...
        delta_time = today_date - early_date
        rand_date = timedelta(days = randint(0, delta_time.days))

        new_date = early_date + rand_date
        return new_date

    elif (not datelist and not surprise):
        delta_time = timedelta(days = 1)
        yest_date = today_date - delta_time
...
def query_url(d, api_key):
    ...
    """
    date_formatted = "&date=" +  d.strftime("%Y-%m-%d")
    url = "https://api.nasa.gov/planetary/apod?api_key="
    
    full_url = url + api_key + date_formatted
    return full_url
